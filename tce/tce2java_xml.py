...
class Builtin_Python:
	...
	@staticmethod
	def unserial(typ,val,xmlnode,sw):
		s=''
...
#	sw.writeln("d.writeInt(this.ds.length);")
	...
	if isinstance( e.type,Builtin):
		pass # Builtin_Python.serial(e.type,'item',sw,'xml')
...
#	sw.writeln("import tce.RpcServantDelegate;")
...
#
	...
	sw.writeln('public static %sProxy createWithHTTP(String host,int port){'%e.getName()).idt_inc()
	sw.writeln('RpcConnection conn = RpcCommunicator.instance().'
...
#			sw.scope_end()
		...
		if m.type.name =='void':
			...
			sw.define_var('r','boolean','false')
			sw.define_var('m','RpcMessageXML','new RpcMessageXML(RpcMessage.CALL|RpcMessage.ONEWAY)')
			sw.writeln('m.msgcls = "%s";'%e.name)
			sw.writeln('m.msg = "%s";'% m.name)
			for p in m.params:
				if isinstance(p.type,Builtin):
		...
		if m.type.name !='void':
			...
			sw.writeln('public void %s_async(%sRpcAsyncCallBackBase async) throws RpcException{'%(m.name,s) ).idt_inc()
			sw.define_var('r','boolean','false')
	...
	for m in e.list: # func
		...
		sw.writeln('public void %s(%s result,RpcProxyBase proxy){'%(m.name,m.type.name)).idt_inc()
		sw.scope_end()
		sw.wln()

	sw.writeln('@Override')
	sw.writeln('public void callReturn(RpcMessage m1_,RpcMessage m2_){').idt_inc()
	sw.writeln('NodeList nodes;')
	sw.writeln('Element node;')
	sw.writeln('boolean r = false;')
